...
void main() {
  group('SoundDetectionThresholds Tests', () {
    ...
    test('Baseline thresholds match specifications for all 9 categories', () {
      expect(SoundDetectionThresholds.getThreshold(SoundCategory.fireAlarm), equals(0.65));
      expect(SoundDetectionThresholds.getThreshold(SoundCategory.smokeAlarm), equals(0.65));
      expect(SoundDetectionThresholds.getThreshold(SoundCategory.emergencySiren), equals(0.70));
      expect(SoundDetectionThresholds.getThreshold(SoundCategory.glassBreaking), equals(0.75));
      expect(SoundDetectionThresholds.getThreshold(SoundCategory.doorbell), equals(0.70));
      expect(SoundDetectionThresholds.getThreshold(SoundCategory.knocking), equals(0.70));
      expect(SoundDetectionThresholds.getThreshold(SoundCategory.babyCrying), equals(0.65));
      expect(SoundDetectionThresholds.getThreshold(SoundCategory.dogBarking), equals(0.75));
      expect(SoundDetectionThresholds.getThreshold(SoundCategory.vehicleHorn), equals(0.75));
    });

    test('meetsThreshold correctly rejects below-threshold confidence', () {
      // Dog Barking threshold is 0.75
      expect(thresholds.meetsThreshold(SoundCategory.dogBarking, 0.74), isFalse);
    ...
    test('meetsThreshold correctly accepts equal or above-threshold confidence', () {
      // Dog Barking threshold is 0.75
      expect(thresholds.meetsThreshold(SoundCategory.dogBarking, 0.75), isTrue);
    ...
    test('Category-specific differentiation: 0.70 satisfies Baby Crying but rejects Dog Barking', () {
      const score = 0.70;
      expect(thresholds.meetsThreshold(SoundCategory.babyCrying, score), isTrue); // req: 0.65
      expect(thresholds.meetsThreshold(SoundCategory.dogBarking, score), isFalse); // req: 0.75
      expect(thresholds.meetsThreshold(SoundCategory.doorbell, score), isTrue); // req: 0.70
    });
    ...
    test('Custom threshold overrides modify specific categories while retaining others', () {
      ...
      // Fire alarm remains default 0.65
      expect(custom.thresholdFor(SoundCategory.fireAlarm), equals(0.65));
    ...
    test('toMap returns all 9 category thresholds and is unmodifiable', () {
      ...
      expect(map[SoundCategory.emergencySiren], equals(0.70));
